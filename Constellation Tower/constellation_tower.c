...
//= 1.2 Fixed Reward Calculations                                =|
//= 1.3 Rebalanced Betel Stat Calculation                        =|
//= 1.4 Fixed qid usage                                          =|
//= 1.5 Added 1000% Update                                       =|
//= 1.6 Added missing dialogues                                  =|
//================================================================|
...
alberta,203,76,5	script	#ctower_vending	HIDDEN_WARP_NPC,6,6,{
	end;
	
OnTouch:
	if ( isbegin_quest(12515) < 2 ) end;
	cloaknpc("OSC0007#ctower", false);
	cloaknpc("OSC0008#ctower", false);
end;
}

alberta,203,76,5	script(CLOAKED)	OSC0008#ctower	2_SLOT_MACHINE,{
	mes "^0000CD- Thanks to your continous support, we were able to open a new branch of vending machines in Alberta. Please do use it a lot! -^000000";
	next;
	.@s = select("Cancel:Nebula of Power:Nebula of Stamina:Nebula of Concentration:Nebula of Creative:Nebula of Spell:Nebula of Wisdom:Signet of Star") - 1;
	if ( .@s == 0 ) end;
	.@id = 6 + .@s;
	item_enchant(.@id);
	end;
...
alberta,206,76,5	script(CLOAKED)	OSC0007#ctower	2_DROP_MACHINE,{
	mes "^0000CD- Thanks to your continous support, we were able to open a new branch of vending machines in Alberta. Please do use it a lot! -^000000";
	next;
	.@s = select("Cancel:Basic Shop:Advanced Shop") - 1;
	if ( .@s == 0 ) end;
	if ( .@s == 1 )
		callshop "ctower_basic";
	else
		callshop "ctower_advanced";
	end;
}
...
OnInstanceInit:
	...
	for ( .@i = 1; .@i <= 5; ++.@i ) {
		instance_enable("#ctower_warp_" + .@i, true);
	}
end;
...
1@ch_t,46,188,6	script(DISABLED)	#oscar_ctower_1	4_ED_OSCAR,{
	cutin "OSCAR01",2;
	mes "[Oscar]";
    if(!is_party_leader()){	
	...
	instance_enable("#ctower_jump",true);
...
1@ch_t,50,188,0	script(DISABLED)	#ctower_jump	CLEAR_NPC,{
...
1@ch_t,13,221,1	script(DISABLED)	#ctower_warp_1	PORTAL,{
...
//Easy Boss
2@ch_t,91,95,7	script(DISABLED)	#ctower_leticia2	4_4JOB_LETICIA,{
	...
	if(.@var == 2){
		if ( checkquest(12519, HUNTING) == 2 ) {
			...
			cutin "4job_leticia_04.png",2;
			mes "[Leticia]";
			mes "Oh, something came out of that thing's body.";
...
OnTouch:
	if ( get_instance_var("mode") ) end;
	if ( isbegin_quest(12519) == 0 )
		setquest 12519;
end;
}

2@ch_t,103,107,3	script	#ctower_naght	MD_NAGHT_SIEGER,{
	end;
...
OnTouch:
	if( !get_instance_var("mode") ) {
		if ( isbegin_quest(12519) ) 
			erasequest 12519;
	} else {
		if ( isbegin_quest(12518) ) 
			erasequest 12518;
	}
end;
}
...
3@ch_t,40,93,6	script	#oscar_ctower_2	4_ED_OSCAR,{
	...
	if( !get_instance_var("switch_active") ) {
		...
		instance_enable("#ctower_betel_0",true);
...
3@ch_t,32,102,1	script(DISABLED)	#ctower_button	CLEAR_NPC,{
...
3@ch_t,45,89,1	script(DISABLED)	#ctower_fountain	CLEAR_NPC,{
	if ( !get_instance_var("mode") ) {
	...
	switch(.@RES){
		...
		case 5: .@TOTAL += 250; break;
	}
	switch(.@TRAP){
		case 1: .@TOTAL += 85; break;
		case 2: .@TOTAL += 170; break;
		case 3: .@TOTAL += 255; break;
		case 4: .@TOTAL += 340; break;
		case 5: .@TOTAL += 425; break;
	}
	...
	switch(
		select(
			"Quit",
			"Randomize Abilities",
			(isbegin_quest(12516) == 2 ? "Maximize an ability" : ""),
			(isbegin_quest(12517) == 2 ? "Seal an ability" : "")
		)
	) {
		case 1:
			break;
		...
		case 2:
			mes "- For the randomizing the constellation's abilities, you will need to offer ^0000CD 20 " + getitemname(1000372) + "^000000.";
			mes "----------------------";
			...
			set_instance_var("HP",rand(.@min,.@max));
			set_instance_var("ATK",rand(.@min,.@max));
			set_instance_var("RECO",rand(.@min,.@max));
			set_instance_var("RES",rand(.@min,.@max));
			set_instance_var("TRAP",rand(.@min,.@max));
			mes "The constellation abilities has changed.";
			break;

		case 3:
			if ( isbegin_quest(12516) < 2 ) {
				mes "- You haven't unlocked the fountains secret. -";
				close;
			}
			mes "- For maximizing an ability of the constellation, you will need to offer ^0000CD 150 " + getitemname(1000372) + "^000000.";
			next;
			.@s = select("Quit", "Maximize Health", "Maximize Attack", "Maximize Recovery", "Maximize Defense", "Maximize Traps") - 1;
			if ( .@s == 0 )
				end;
			switch ( .@s ) {
				case 1: .@var$ = "HP"; .@title$ = "Health"; break;
				case 2: .@var$ = "ATK"; .@title$ = "Attack"; break;
				case 3: .@var$ = "RECO"; .@title$ = "Recovery"; break;
				case 4: .@var$ = "RES"; .@title$ = "Defense"; break;
				case 5: .@var$ = "TRAP"; .@title$ = "Traps"; break;
			}
			.@max = (get_instance_var("mode") < 2 ? 3 : 5);
			if ( get_instance_var(.@var$) == .@max ) {
				mes "- The " + .@title$ + " has already been maximized. -";
				close;
			}
			mes "- Are you sure you want to maximize the " + .@title$ + " ability of the constellation? -";
			next;
			if ( select("No:Yes") == 1 )
				end;
			if(countitem(1000372) < 150){
				mes "- You don't have the required amount of ^FF0000" + getitemname(1000372) + "^000000 for the offering.";
				end;
			}
			delitem 1000372,150;
			set_instance_var(.@var$, .@max);
			mes "The constellation's " + .@title$ + " ability has been maximized.";
			break;
		...
		case 4:
			...
			mes "- For sealing an ability of the constellation, you will need to offer ^0000CD 300 " + getitemname(1000372) + "^000000.";
			next;
			.@s = select("Quit", "Seal Health", "Seal Attack", "Seal Recovery", "Seal Defense", "Seal Traps") - 1;
			if ( .@s == 0 )
				end;
			switch ( .@s ) {
				case 1: .@var$ = "HP"; .@title$ = "Health"; break;
			...
			.@min = (get_instance_var("mode") < 2 ? 0 : 3);
			if ( get_instance_var(.@var$) == .@min ) {
				mes "- The " + .@title$ + " has already been sealed. -";
				close;
			}
			mes "- Are you sure you want to maximize the " + .@title$ + " ability of the constellation? -";
			next;
			...
			if(countitem(1000372) < 300){
				mes "- You don't have the required amount of ^FF0000" + getitemname(1000372) + "^000000 for the offering.";
				end;
			}
			delitem 1000372,300;
			set_instance_var(.@var$, .@min);
			mes "The constellation's " + .@title$ + " ability has been sealed.";
			break;
	}
	.@HP = get_instance_var("HP");
	.@ATK = get_instance_var("ATK");
...
3@ch_t,156,86,5	script(DISABLED)	#ctower_betel_0	MD_BETELGEUSE,{
	if(!is_party_leader()){
	...
	if(!get_instance_var("phase")){
		...
		if(isbegin_quest(12513) == 1){
			...
			npctalk "Most of my abilities are still sealed, but I think your power is still not enough to stop me..";
			sleep 3000;
			specialeffect EF_MAGIC_FLOOR;
			sleep 3000;
		}
...
3@ch_t,156,86,5	script(DISABLED)	#ctower_betel_1	MD_BETELGEUSE,{ end; }
3@ch_t,156,86,5	script(DISABLED)	#ctower_betel_2	MD_BETELGEUSE,{ end; }

3@ch_t,156,86,5	script(DISABLED)	#ctower_betel_3	MD_BETELGEUSE,{
	if(get_instance_var("phase") == 4){
		if ( isbegin_quest(12515) == 0 ) {
			...
			mes "[Betelguese]";
			mes "I just hope you the red one, stop paying attention to me and leave me alone~";
			next;
			cutin "4job_leticia_03.png",0;
			mes "[Leticia]";
		...
		if( checkquest(12518, HUNTING) == 2 ) {
			...
			if ( .@total >= 300 ) {
				if ( isbegin_quest(12516) == 0 ) {
					setquest 12516;
					completequest 12516;
				}
			}
			if ( .@total >= 600 ) {
				if ( isbegin_quest(12517) == 0 ) {
					setquest 12517;
					completequest 12517;
				}
			}
...
function	script	ctower_calc_stats	{
	...
	if( unitexists(.@gid) ){   
		...
		.@RES = get_instance_var("RES");
		if( .@HP ){
			.@MOB_HP = min(2000000000, .@data[UMOB_MAXHP] + (300000000 * .@HP));
		...
		if( .@ATK ){
			...
			.@MOB_ATK = min(65000, .@data[UMOB_ATKMIN] + (5000 * .@ATK));
			.@MOB_ATK2 = min(65000, .@data[UMOB_ATKMAX] + (5000 * .@ATK));
			
			.@MOB_MATK = min(65000, .@data[UMOB_MATKMIN] + (5000 * .@ATK));
			.@MOB_MATK2 = min(65000, .@data[UMOB_MATKMAX] + (5000 * .@ATK));
			
			.@STR = .@data[UMOB_STR] + (60 * .@ATK);
			.@AGI = .@data[UMOB_AGI] + (60 * .@ATK);
			.@INT = .@data[UMOB_INT] + (60 * .@ATK);
			.@VIT = .@data[UMOB_VIT] + (60 * .@ATK);
			.@DEX = .@data[UMOB_DEX] + (60 * .@ATK);
			.@LUK = .@data[UMOB_LUK] + (60 * .@ATK);
			
			setunitdata .@gid, UMOB_ATKMIN, .@MOB_ATK;
			setunitdata .@gid, UMOB_ATKMAX, .@MOB_ATK2;
			setunitdata .@gid, UMOB_MATKMIN, .@MOB_MATK;
			setunitdata .@gid, UMOB_MATKMAX, .@MOB_MATK2;
			...
			set_instance_var("BOSS_MATK", .@MOB_MATK);
			set_instance_var("BOSS_MATK2", .@MOB_MATK2);
			set_instance_var("BOSS_STR", .@STR);
			set_instance_var("BOSS_AGI", .@AGI);
			set_instance_var("BOSS_INT", .@INT);
			set_instance_var("BOSS_VIT", .@VIT);
			set_instance_var("BOSS_DEX", .@DEX);
			set_instance_var("BOSS_LUK", .@LUK);
		}
		if( .@RES ) {
			.@RES = .@data[UMOB_RES] + (300 * .@RES);
			.@MRES = .@data[UMOB_MRES] + (300 * .@MRES);
...
function	script	ctower_revert_stats	{
	...
	if( unitexists(.@gid) ) {
		...
		setunitdata .@gid, UMOB_AGI, get_instance_var("BOSS_AGI");
		setunitdata .@gid, UMOB_VIT, get_instance_var("BOSS_VIT");
		setunitdata .@gid, UMOB_INT, get_instance_var("BOSS_INT");
		setunitdata .@gid, UMOB_DEX, get_instance_var("BOSS_DEX");
		setunitdata .@gid, UMOB_LUK, get_instance_var("BOSS_LUK");
	}
	return;
}
...
function	script	ctower_recalc_stats	{
    ...
    if(unitexists(.@gid)){
        ...
        .@MOB_ATK2 = get_instance_var("BOSS_ATK2") + (5000 * .@value);
		.@MOB_MATK = get_instance_var("BOSS_MATK") + (5000 * .@value);
		.@MOB_MATK2 = get_instance_var("BOSS_MATK2") + (5000 * .@value);

        .@STR = get_instance_var("BOSS_STR") + (60 * .@value);
        .@AGI = get_instance_var("BOSS_AGI") + (60 * .@value);
        .@VIT = get_instance_var("BOSS_VIT") + (60 * .@value);
        .@INT = get_instance_var("BOSS_INT") + (60 * .@value);
        .@DEX = get_instance_var("BOSS_DEX") + (60 * .@value);
        .@LUK = get_instance_var("BOSS_LUK") + (60 * .@value);
...
OnSecondPhaseCheck:
    ...
    while ( unitexists(.@gid) ) {
        getunitdata .@gid,.@data;
        if ( .@data[UMOB_HP] <= (.@data[UMOB_MAXHP]/10) * 5 ) {
...
OnBossKill:
	...
	if(.@AMOUNT){
		...
		if ( .@TOTAL >= 850 )
			instance_enable("#ctower_treasure_13",true);
	}
end;
}

3@ch_t,147,76,3	script(DISABLED)	#ctower_treasure_1	4_STAR_BOX_POW1,{
	.@id = atoi(replacestr(strnpcinfo(2),"ctower_treasure_",""));
	.@total = get_instance_var("TOTAL");
	.@s_min = (.@total <= 600 ? 5 : 10);
	.@s_max = (.@total <= 600 ? 10 : 15);
	.@h_min = (.@total <= 600 ? 3 : 5);
	.@h_max = (.@total <= 600 ? 5 : 8);
	.@c_min = (.@total < 850 ? 7 : 12);
	.@c_max = (.@total < 850 ? 10 : 15);
	switch ( .@id ) {
		case 1: .@item_id = 1000398; .@amount = rand(.@s_min,.@s_max); break;
		case 2: .@item_id = 1000399; .@amount = rand(.@s_min,.@s_max); break;
		case 3: .@item_id = 1000402; .@amount = rand(.@s_min,.@s_max); break;
		case 4: .@item_id = 1000400; .@amount = rand(.@s_min,.@s_max); break;
		case 5: .@item_id = 1000403; .@amount = rand(.@s_min,.@s_max); break;
		case 6: .@item_id = 1000442; .@amount = rand(.@h_min,.@h_max); break;
		case 7: .@item_id = 1000443; .@amount = rand(.@h_min,.@h_max); break;
		case 8: .@item_id = 1000446; .@amount = rand(.@h_min,.@h_max); break;
		case 9: .@item_id = 1000444; .@amount = rand(.@h_min,.@h_max); break;
		case 10: .@item_id = 1000447; .@amount = rand(.@h_min,.@h_max); break;
		case 11: .@item_id = 1000401; .@amount = rand(.@h_min,.@h_max); break;
		case 12: .@item_id = 1000401; .@amount = rand(.@c_min,.@c_max); break;
		case 13: .@item_id = 1000373; .@amount = rand(15,20); break;
	}
...
3@ch_t,151,80,3	duplicate(#ctower_treasure_1)	#ctower_treasure_13	4_STAR_BOX_CRT1
...
OnTimer5000:
	...
	getunitdata .@gid,.@data;
	//RECO * 2% (MAX_HP)
	.@HEAL = (.@data[UMOB_MAXHP]/100) * (get_instance_var("RECO") * 2);
...
3@ch_t,142,85,3	script(DISABLED)	#ctower_trap_0	MD_MANHOLE,1,1,{
    end;
...
3@ch_t,145,85,3	duplicate(#ctower_trap_0)	#ctower_trap_1	MD_MANHOLE,1,1
3@ch_t,148,85,3	duplicate(#ctower_trap_0)	#ctower_trap_2	MD_MANHOLE,1,1
3@ch_t,151,85,3	duplicate(#ctower_trap_0)	#ctower_trap_3	MD_MANHOLE,1,1
3@ch_t,154,85,3	duplicate(#ctower_trap_0)	#ctower_trap_4	MD_MANHOLE,1,1
3@ch_t,157,85,3	duplicate(#ctower_trap_0)	#ctower_trap_5	MD_MANHOLE,1,1
3@ch_t,160,85,3	duplicate(#ctower_trap_0)	#ctower_trap_6	MD_MANHOLE,1,1
3@ch_t,163,85,3	duplicate(#ctower_trap_0)	#ctower_trap_7	MD_MANHOLE,1,1
3@ch_t,166,85,3	duplicate(#ctower_trap_0)	#ctower_trap_8	MD_MANHOLE,1,1
3@ch_t,169,85,3	duplicate(#ctower_trap_0)	#ctower_trap_9	MD_MANHOLE,1,1

3@ch_t,156,64,3	script(DISABLED)	#ctower_trap2_0	MD_MANHOLE,1,1,{
    end;
...
3@ch_t,156,67,3	duplicate(#ctower_trap2_0)	#ctower_trap2_1	MD_MANHOLE,1,1
3@ch_t,156,70,3	duplicate(#ctower_trap2_0)	#ctower_trap2_2	MD_MANHOLE,1,1
3@ch_t,156,73,3	duplicate(#ctower_trap2_0)	#ctower_trap2_3	MD_MANHOLE,1,1
3@ch_t,156,76,3	duplicate(#ctower_trap2_0)	#ctower_trap2_4	MD_MANHOLE,1,1
3@ch_t,156,79,3	duplicate(#ctower_trap2_0)	#ctower_trap2_5	MD_MANHOLE,1,1
3@ch_t,156,82,3	duplicate(#ctower_trap2_0)	#ctower_trap2_6	MD_MANHOLE,1,1
3@ch_t,156,85,3	duplicate(#ctower_trap2_0)	#ctower_trap2_7	MD_MANHOLE,1,1
3@ch_t,156,88,3	duplicate(#ctower_trap2_0)	#ctower_trap2_8	MD_MANHOLE,1,1
3@ch_t,156,91,3	duplicate(#ctower_trap2_0)	#ctower_trap2_9	MD_MANHOLE,1,1
3@ch_t,156,94,3	duplicate(#ctower_trap2_0)	#ctower_trap2_10	MD_MANHOLE,1,1
3@ch_t,156,97,3	duplicate(#ctower_trap2_0)	#ctower_trap2_11	MD_MANHOLE,1,1
3@ch_t,156,100,3	duplicate(#ctower_trap2_0)	#ctower_trap2_12	MD_MANHOLE,1,1
3@ch_t,156,103,3	duplicate(#ctower_trap2_0)	#ctower_trap2_13	MD_MANHOLE,1,1
3@ch_t,156,106,3	duplicate(#ctower_trap2_0)	#ctower_trap2_14	MD_MANHOLE,1,1
3@ch_t,156,109,3	duplicate(#ctower_trap2_0)	#ctower_trap2_15	MD_MANHOLE,1,1

3@ch_t,156,96,1	script(DISABLED)	#ctower_exit	PORTAL,{
	mes "^0000CDDo you want to leave the tower?^000000";
	next;
	if(select("Cancel.:Leave.") == 1) end;
	mes "^0000CDAn unknown force pulled you out from the tower and blasted you into Alberta.^000000";
	warp "alberta",223,36;
	end;
}